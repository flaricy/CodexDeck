...
Pending-question integration evidence: the official [App Server documentation](https://learn.chatgpt.com/docs/app-server) documents runtime `thread/status/changed`, `item/tool/requestUserInput`, and `serverRequest/resolved`. A read-only probe of a separately launched installed app-server returned zero loaded threads while desktop work was active. Thus launching another server does not provide the desktop process's live event stream. The adapter needs an authorized connection to the owning desktop server; no private enrollment credentials are reused. This remains an explicit acceptance gap, not a classifier-only task.
